...
cd tesseract;
git checkout 3.04.01; # (head of tree broke Jan2017)
./autogen.sh;
./configure --disable-shared --enable-static;  make;  make install;
cd -;


# ccextractor -- build static -- pick stable release or head of tree:
if [ "1" ]; then
  git clone https://github.com/CCExtractor/ccextractor;
else
  wget https://sourceforge.net/projects/ccextractor/files/ccextractor/0.84/ccextractor.src.0.84.zip;
